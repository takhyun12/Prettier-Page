...
class PrettierPage:
    def __init__(self, image_path):
        # Input Check
        if not image_path:
            print('[>] 정상적인 파일입력이 아닙니다.')
            return

        # Parse image name and extension
        image_name = os.path.basename(image_path)  # type: str
        image_extension = os.path.splitext(image_name)[1].upper()  # type: str

        # Extension Check for Prevent Exception
        allow_extension_list = [".TIF", ".GIF", ".JPG", ".JPEG"]  # type: List[str]

        # Image Read and assignment self variable
    ...
    def Make_Pretty(self):
        # Image Resizing
        ratio = self.source_image.shape[0] / 500.0
